...
export class PaymentController {
	...
	async getAll(req, res) {
		try {
			...
			defaultReturnStatement(res, "Payments", Promise.resolve(payments));
		} catch (err) {
			console.error(err);
	...
	async createPayment(req, res) {
		try {
			...
			defaultReturnStatement(res, "Payment Created", Promise.resolve(payment));
		} catch (err) {
			console.error(err);
			res.status(500).json({ status: 500, error: err });
		}
	...
	async updatePayment(req, res) {
		try {
			...
			defaultReturnStatement(res, "Payment and Order updated successfully", Promise.resolve(result));
		} catch (err) {
			console.error(err);
